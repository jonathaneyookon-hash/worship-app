...
function startServer({ state, search, port = 3939 }) {
  ...
  app.get('/api/health', (req, res) => res.json({ ok: true, service: 'Scripture Remote', output: state.snapshot().output }));
  ...
  app.get('/api/search', (req, res) => {
    ...
    try { return res.json(search.search(String(q), String(version))); }
    catch (e) { return res.status(500).json({ error: e.message }); }
  });

  app.get('/api/search-all', (req, res) => {
    const ref = search.parseReference(String(req.query.q || ''));
    if (!ref) return res.status(400).json({ error: 'Could not parse a reference from that input.' });
    return res.json({ reference: ref, versions: search.getByReferenceAllVersions(ref) });
  });

  app.post('/api/schedule', (req, res) => {
    const entry = state.addToSchedule(req.body);
    if (!entry) return res.status(400).json({ error: 'A schedule item requires reference and text.' });
  ...
  app.post('/api/preview', (req, res) => { state.setPreview(req.body); res.json({ ok: true }); });
  app.post('/api/go-live', (req, res) => { state.goLive(req.body); res.json({ ok: true }); });
  app.post('/api/black', (req, res) => { state.blackout(); res.json({ ok: true, output: 'black' }); });
  app.post('/api/restore', (req, res) => { state.restoreLive(); res.json({ ok: true, output: state.snapshot().output }); });
  app.post('/api/clear', (req, res) => { state.clearLive(); res.json({ ok: true, output: 'clear' }); });
