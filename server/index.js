...
function startServer({state,search,port=3939}){
 ...
 const resolveItem=(item)=>{if(!item||!item.reference)return item;const ref=search.parseReference(String(item.reference));if(!ref)return item;const rows=search.getByReference(ref,state.snapshot().selectedVersion);if(!rows.length)return item;return {...item,version:state.snapshot().selectedVersion,text:rows.map(r=>r.text).join(' ')};};
 ...
 app.post('/api/schedule',(req,res)=>{const entry=state.addToSchedule(resolveItem(req.body));if(!entry)return res.status(400).json({error:'A schedule item requires reference and text.'});return res.json(entry);});
